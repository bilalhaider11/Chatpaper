...
The `backend/` directory is bind-mounted into the `backend`, `celery_worker`, and `celery_beat` containers. Changes to any `.py` file are picked up automatically:

- **backend** — uvicorn runs with `--reload`; reloads within ~1 s of a save
- **celery_worker / celery_beat** — `watchmedo auto-restart` (from `watchdog`) restarts the process on any `.py` change

| Situation | Command |
|---|---|
| First-time setup | `docker compose up --build` |
| Normal start (stack was stopped) | `docker compose up -d` |
| `.py` file changed | nothing — auto-reloaded |
| `requirements.txt` or `Dockerfile` changed — **infra already running** | `docker compose up -d --build --no-deps backend celery_worker celery_beat` |
| `requirements.txt` or `Dockerfile` changed — **starting from scratch** | `docker compose up -d --build` |

> `--no-deps` skips starting `db`, `redis`, `rabbitmq`, and `chromadb`. Only use it when those services are already up.
...
Available at `http://localhost:8000/admin`. Login uses `ADMIN_USERNAME` / `ADMIN_PASSWORD` (separate from user JWT auth). Brute-force protected: 10 failed attempts from an IP triggers a 5-minute lockout via Redis.

Registered views:

| View | Table | Notable columns |
|---|---|---|
| User | `users` | id, email, role, is_active |
| File | `files_data` | id, user_id, filename, file_type, filesize, ingestion_status, total_pages |
| Ingestion Job | `ingestion_jobs` | id, file_id, status, current_stage/total_stages, retry_count, error_type |
| Conversation | `conversationlist` | id, user_id, title, type (global/per_file), file_id |
| Message | `conversation` | id, chat_id, user_type (human/ai), statement |

Password and raw file path fields are excluded from list views.
...
| Admin panel exposure | `AuthenticationBackend` on sqladmin; password fields excluded from all column lists; rate-limited login (10 attempts → 5 min lockout per IP) |
...
| `python-jose` / `bcrypt` | JWT creation/validation and password hashing |
